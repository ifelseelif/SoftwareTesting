...
public class BasicTrigFunctionIntegrationTest {
    ...
    public void testCosineIntegration(double value) {
        ...
        when(sine.calculate(anyDouble())).thenReturn(Math.sin(value));

        Cosine cosine = new Cosine(sine);

        Assertions.assertEquals(Math.cos(value), cosine.calculate(value), DELTA);
    }
    ...
    public void testSecantIntegration(double value) {
        ...
        when(cosine.calculate(anyDouble())).thenReturn(Math.cos(value));

        Secant secant = new Secant(cosine);

        if (value == Math.PI / 2) {
            Assertions.assertEquals(Double.POSITIVE_INFINITY, secant.calculate(value), DELTA);
        } else {
            Assertions.assertEquals(1 / Math.cos(value), secant.calculate(value), DELTA);
        }
    }
    ...
    public void testCosecantIntegration(double value) {
        ...
        Assertions.assertEquals(1 / Math.sin(value), cosecant.calculate(value), DELTA);
    }
    ...
    public void testTangentIntegration(double value) {
        ...
        Assertions.assertEquals(Math.tan(value), tangent.calculate(value), DELTA);
    }
    ...
    public void testCotangentIntegration(double value) {
        ...
        Assertions.assertEquals(1 / Math.tan(value), cotangent.calculate(value), DELTA);
    }
